    ...
    for (const emails of emailsArray) {
        ...
        for (const email of emails) {
            try {
                ...
                console.log(`[ERROR] users.lookupByEmail APIの実行エラー email: ${email} err: ${err}`);
            }
        }
        ...
        try {
            const conversationsOpenResult = (await web.conversations.open
                ({ users : usersString})) as ConversationsOpenResult;

            try {
                (await web.chat.postMessage
                    ({ channel : conversationsOpenResult.channel.id , text : text}));
                console.log(`[INFO] emails: ${emails.join(',')} へのDM送信に成功しました。`);
            } catch (err) {
                console.log(`[ERROR] chat.postMessage APIの実行エラー channel: ${conversationsOpenResult.channel.id} err: ${err}`);
            }

        } catch (err) {
            console.log(`[ERROR] conversations.open APIの実行エラー usersString: ${usersString} err: ${err}`);
        }
        counter++;
    }
